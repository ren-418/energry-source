...
const Register: React.FC = () => {
    ...
    const [utilities, setUtilities] = useState<{name: string, logo: string}[]>([]);
        ...
        <div className="w-[100vw] h-[100vh]">
            <div className="w-full h-full bg-black/80 flex justify-center items-center">
                <div className="flex flex-col w-full md:w-[400px] h-full md:h-[750px] px-[16px] py-[16px] md:rounded-lg bg-[#f5f5f7] z-10 overflow-auto">
                    {currentStep === 'area' && <StepArea zipcode={zipcode} setZipcode={setZipcode} email={email} setEmail={setEmail} handleNextStep={handleNextStep} setUtilities={setUtilities} />}
                    {currentStep === 'utility' && <StepUtility handleNextStep={handleNextStep} handlePreviousStep={handlePreviousStep} setCurrentStep={setCurrentStep} setSelectedUtility={setSelectedUtility} zipcode={zipcode} utilities={utilities} />}
